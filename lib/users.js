...
/** שמות כל המנהלים כפי שהם רשומים בגיליון. משמש לניתוב התראות. */
export async function adminNames() {
  const users = await allUsers();
  const names = users.filter(u => u.role === "מנהל" && u.active).map(u => u.name).filter(Boolean);
  return names.length ? names : ["רון"];
}
