...
export function pool(records, scope) {
  return records.filter(r => {
    ...
    if (scope && scope !== "all") {
      ...
      // תאימות לאחור: "עדיין לא לקוחה" מכסה את שני שלבי הוובינר עד שיתויג מחדש
      const legacyPre = types.includes("עדיין לא לקוחה") &&
        (scope === "לא השתתפה בוובינר" || scope === "השתתפה בוובינר");
      if (types.length && !universal && !legacyPre && !types.includes(scope)) return false;
    }
    return true;
  });
}

/** נרמול עמוק: מוריד אותיות סופיות, אמות קריאה כפולות ופיסוק, כדי לבלוע שגיאות כתיב קלות */
function deepNorm(s) {
  return norm(s)
    .replace(/ם/g, "מ").replace(/ן/g, "נ").replace(/ץ/g, "צ")
    .replace(/ך/g, "כ").replace(/ף/g, "פ")
    .replace(/וו/g, "ו").replace(/יי/g, "י")
    .replace(/[הא]/g, "")          // אמות קריאה שנוטות להשתנות בכתיב
    .replace(/\s+/g, " ").trim();
}

/** מרחק עריכה מוגבל: מחזיר true אם צריך עד max שינויים כדי להשוות בין שתי מחרוזות */
function closeEnough(a, b, max) {
  ...
  for (let i = 1; i <= a.length; i++) {
    ...
    if (rowMin > max) return false;   // אין סיכוי להגיע מתחת לסף
  }
  return dp[a.length][b.length] <= max;
}

/**
 * שלב 1: התאמה מדויקת, סלחנית לשגיאות כתיב.
 * קודם השוואה רגילה, אחר כך נרמול עמוק, ולבסוף סטייה של עד שני תווים.
 */
export function exactMatch(records, query, scope) {
  const q = norm(query);
  const list = pool(records, scope);

  const texts = r => [r["שאלה מרכזית"], ...(r["ניסוחים חלופיים"] || "").split(";")]
    .filter(t => t && t.trim());

  // זהות מלאה
  let hit = list.find(r => texts(r).some(t => norm(t) === q));
  if (hit) return hit;

  // זהות אחרי נרמול עמוק
  const dq = deepNorm(query);
  hit = list.find(r => texts(r).some(t => deepNorm(t) === dq));
  if (hit) return hit;

  // סטייה קטנה, עד שני תווים, ורק על טקסט קצר יחסית
  if (dq.length <= 60) {
    const tolerance = dq.length <= 20 ? 1 : 2;
    hit = list.find(r => texts(r).some(t => {
      const dt = deepNorm(t);
      return Math.abs(dt.length - dq.length) <= tolerance && closeEnough(dq, dt, tolerance);
    }));
    if (hit) return hit;
  }
  return null;
...
/**
 * מדרג את הרשומות לפי קרבה להודעה ומחזיר את המובילות בלבד.
 * משמש כדי לשלוח למנוע רשימה קצרה במקום את כל המאגר, וכך לקצר את זמן התגובה.
 */
export function topCandidates(records, query, scope, limit = 60) {
  const list = pool(records, scope);
  const qw = new Set(words(query));
  if (!qw.size) return list.slice(0, limit);
  ...
  const scored = list.map(r => {
    ...
    let inter = 0; qw.forEach(w => { if (hay.has(w)) inter++; });
    return { r, score: hay.size ? (2 * inter) / (qw.size + hay.size) : 0 };
  });

  scored.sort((a, b) => b.score - a.score);
  const top = scored.slice(0, limit).map(x => x.r);

  // תמיד לצרף נוסחים כלליים, כדי שיהיה על מה ליפול
  const generals = records.filter(r => r["סטטוס"] === "מאושר" && isGeneral(r));
  for (const g of generals) if (!top.includes(g)) top.push(g);
  return top;
}
